    ...
    async def process_mm_data_async(
        ...
        **kwargs
        ...
        if images_are_preprocessed:
            ...
            processed_text = base_output.input_text.replace(self.IMG_CONTEXT_TOKEN, original_placeholder)
            ...
            for idx, num_patches in enumerate(num_patches_list):
                ...
                processed_text = processed_text.replace(original_placeholder, image_tokens, 1)

            processed_text = processed_text.replace(original_placeholder, self.IMG_CONTEXT_TOKEN)

            input_ids = self.tokenizer(processed_text, return_tensors="pt")[
                "input_ids"
            ].flatten()
            image_offsets = self.get_mm_items_offset(
                input_ids=input_ids,
